...
export class ProductServiceHelper {
    ...
    protected addProductToStorage(id: number) {
        ...
        if (storage != null && storage != '') {
            ...
            localStorage.setItem('beaz_cart', JSON.stringify(items));
            this.storageSub.next(JSON.stringify(storage));
        } else {
            localStorage.setItem('beaz_cart', JSON.stringify([id.toString()]));
            this.storageSub.next(JSON.stringify(storage));
        }
    ...
    protected getProductsFromStorage() {
        return localStorage.getItem('beaz_cart') != null ? localStorage.getItem('beaz_cart') : '';
    }
    ...
    protected addWishlistProductToStorage(id: number) {
        ...
        if (storage != null && storage != '') {
            items = JSON.parse(storage);
            items.push(id.toString());
            localStorage.setItem('beaz_wishlist', JSON.stringify(items));
            this.storageSub.next(JSON.stringify(storage));
        } else {
            localStorage.setItem('beaz_wishlist', JSON.stringify([id.toString()]));
            this.storageSub.next(JSON.stringify(storage));
        }
    ...
    protected getWishlistProductsFromStorage() {
        return localStorage.getItem('beaz_wishlist') != null ? localStorage.getItem('beaz_wishlist') : '';
    }
